...
chrome.history.onVisited.addListener(function(historyItem) {
	...
	if (userId) {
		if (historyItem.url.startsWith("")) {
			const apiURL = "https://vitalizemind-nodeapi.onrender.com/addBrowserSearch"
			const searchData = {
			...
			.then(data => {
				...
				if (data.score < 0) {
					...
					const apiURL2 = 'https://vitalizemind-nodeapi.onrender.com/user/sendAlert'
...
chrome.notifications.onClicked.addListener(() => {
	...
	chrome.tabs.create({ url: `https://vitalizemind.onrender.com/dashboard?user_id=${userId}&user_name=${userName}` })
})
